...
(function ($, Drupal, window, document) {
  ...
  Drupal.behaviors.booking_timeslots = {
  ...
  function GetGreenBoxInfo(td) {
      return JSON.parse(decodeURIComponent($(td)[0].id));
  }

  function GetNearHour(td, y) {
    var result = {};
    ...
    for (var k = 0; k < tds.length; k++) {
        if (y >= tds[k].y && y <= tds[k].yl) {
            ...
            //TODO console.log(halfway);
            //halfway.yObj.hm existe, halfway.yObj. end hour n'existe pas
  ...
  function GreenBoxShowArrow (result, showPlaceholder) {
    ...
    console.log(window.GreenBoxItem);
    ...
    window.GreenBoxItem.onclick = function () {
        ...
        height = 30;
console.log(JSON.stringify([info.primary_id, info.secondary_id, info.tertiary_id, info.cid, slot.startDate, height]));
console.log(args);
        //ROMAIN
