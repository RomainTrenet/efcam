  ...
  <table class="full">
    ...
    <thead>
      ...
      <tr>
        ...
        <?php foreach ($columns as $column): ?>
        <th class="item"><?php print $column; ?></th>
        <?php endforeach; ?>
      </tr>
    </thead>
    <tbody>
      <?php foreach ($rows['items'] as $hour): if (isset($hour['hide'])) {continue;} ?>
          <tr class="<?php if (substr($hour['hour'], 3, 2) == 0): ?>zero-minutes<?php endif; ?>">
            ...
            <?php if (!@$hour['values'][0]['hidden']): ?>
              <td <?php if(isset($hour['values'][0]['attributes'])): ?> <?php echo $hour['values'][0]['attributes']; ?> <?php endif; ?> <?php if (isset($hour['values'][0]['records'][0]['rowspan'])): ?> rowspan="<?php echo $hour['values'][0]['records'][0]['rowspan'] ?>" <?php endif; ?>  class="item single-day  <?php if (isset($hour['values'][0]['class'])) echo $hour['values'][0]['class'] ?>" >
